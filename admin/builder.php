                ...
                <div class="bm-card-body">
                    <select id="page-select" class="bm-input bm-select" style="margin-bottom:10px;">
                        ...
                        <?php foreach ($all_pages as $page) : ?>
                            <option value="<?php echo $page->ID; ?>" data-title="<?php echo esc_attr($page->post_title); ?>">
        ...
        <div class="bm-card bm-card-structure">
                    ...
                    <p class="bm-card-subtitle">
                        ...
                        <?php if (!empty($supported_langs)) : ?>
                            ...
                            <?php foreach ($supported_langs as $i => $l) : ?>
                                <code class="bm-lang-mini"><?php echo esc_html($l['code']); ?></code><?php echo $i < count($supported_langs)-1 ? ' ' : ''; ?>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </p>
                </div>
            ...
            <ul id="menux-sortable" style="list-style:none; padding:0; margin:0; min-height:50px;">
                <?php foreach ($items as $item) :
                    ...
                    $icon_val  = esc_attr($item['icon'] ?? '');
                    $vis       = $item['visibility'] ?? 'all';
                    $badge_val = esc_attr($item['badge'] ?? '');
                    $badge_bg  = esc_attr($item['badge_bg'] ?? '#ef4444');
                    $target    = $item['target'] ?? '';
                    $item_key  = esc_attr($item['item_key'] ?? ($is_page ? 'page_'.($item['id']??0) : 'c_'.md5($item['url']??'')));
                    $has_subs  = !empty($item['children']);
                ?>
                <li class="menux-item<?php echo $has_subs ? ' menux-item-has-subs' : ''; ?>">
                    ...
                    <div style="flex:1; min-width:0;">
                        <div style="font-weight:600; margin-bottom:4px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:12px; color:#3c434a;">
                            ...
                            <?php if ($has_subs): ?><span style="font-size:10px; background:#e0f2fe; color:#0369a1; padding:1px 5px; border-radius:8px; margin-left:4px;"><?php echo count($item['children']); ?> children</span><?php endif; ?>
                        </div>
                        <div class="bm-inputs-row">
                            <input type="hidden" name="menu_items[<?php echo $idx; ?>][type]" value="<?php echo esc_attr($item['type']); ?>">
                            <input type="hidden" name="menu_items[<?php echo $idx; ?>][item_key]" value="<?php echo $item_key; ?>">
                            <?php if ($is_page) : ?>
                                <input type="hidden" name="menu_items[<?php echo $idx; ?>][id]" value="<?php echo intval($item['id']); ?>">
                            <?php else : ?>
                                <input type="hidden" name="menu_items[<?php echo $idx; ?>][url]" value="<?php echo esc_url($item['url']); ?>">
                            <?php endif; ?>
                            <?php if (!empty($supported_langs)) : ?>
                                <?php foreach ($supported_langs as $lang) :
                                    $key   = menux_code_to_key($lang['code']);
                                ...
                                ?>
                                <input type="text"
                                       name="menu_items[<?php echo $idx; ?>][<?php echo esc_attr($key); ?>]"
                                ...
                                <?php endforeach; ?>
                            <?php else : ?>
                                <input type="text" name="menu_items[<?php echo $idx; ?>][lang_it_IT]"
                                       value="<?php echo esc_attr($item['lang_it_IT'] ?? $item['it'] ?? ($is_page ? $title : '')); ?>"
                                       placeholder="it-IT" class="bm-lang-input" title="Italiano">
                                <input type="text" name="menu_items[<?php echo $idx; ?>][lang_en_US]"
                                       value="<?php echo esc_attr($item['lang_en_US'] ?? $item['en'] ?? ''); ?>"
                                       placeholder="en-US" class="bm-lang-input" title="English">
                            <?php endif; ?>
                            <div style="display:inline-flex;align-items:center;gap:3px;">
                                <?php if (!empty($icon_val)): ?><i class="bm-icon-preview-el <?php echo $icon_val; ?>" style="font-size:16px;color:#374151;"></i><?php endif; ?>
                                <input type="text" name="menu_items[<?php echo $idx; ?>][icon]" value="<?php echo $icon_val; ?>" placeholder="icon" title="Icon class" class="bm-lang-input bm-icon-input" style="max-width:120px!important;">
                            ...
                            <!-- Badge -->
                            <input type="text" name="menu_items[<?php echo $idx; ?>][badge]" value="<?php echo $badge_val; ?>" placeholder="🏷️ Badge" title="Es: New, Hot, Sale" class="bm-lang-input" style="max-width:64px!important;">
                            <input type="color" name="menu_items[<?php echo $idx; ?>][badge_bg]" value="<?php echo $badge_bg; ?>" title="Badge background color" style="width:30px;height:26px;padding:1px;border:1px solid #ccc;border-radius:3px;cursor:pointer;">
                            <!-- Visibility (WP roles) -->
                            <select name="menu_items[<?php echo $idx; ?>][visibility]" class="menux-visibility">
                            ...
                            <!-- Target -->
                            <select name="menu_items[<?php echo $idx; ?>][target]" style="font-size:11px;height:26px;padding:2px 4px;min-width:75px;">
                            ...
                            <label title="Show notification dot" style="display:flex;align-items:center;gap:3px;font-size:11px;color:#555;cursor:pointer;white-space:nowrap;">
                                <input type="checkbox" name="menu_items[<?php echo $idx; ?>][notif_dot]" value="1" <?php checked($item['notif_dot'] ?? '0', '1'); ?>>
                                🔴
                            </label>
                            <!-- ──── FEATURE 11: Location ──── -->
                            <select name="menu_items[<?php echo $idx; ?>][menu_location]" style="font-size:11px;height:26px;padding:2px 4px;min-width:75px;" title="Menu location">
                        ...
                        <details style="margin-top:4px;<?php echo $has_advanced ? '' : ''; ?>" <?php echo $has_advanced ? 'open' : ''; ?>>
                            ...
                            <div class="bm-inputs-row" style="margin-top:6px;">
                                ...
                                <input type="datetime-local" name="menu_items[<?php echo $idx; ?>][schedule_start]" value="<?php echo esc_attr($item['schedule_start'] ?? ''); ?>" style="font-size:10px;height:24px;padding:2px 4px;max-width:160px;">
                                <label style="font-size:10px;color:#555;">to:</label>
                                <input type="datetime-local" name="menu_items[<?php echo $idx; ?>][schedule_end]" value="<?php echo esc_attr($item['schedule_end'] ?? ''); ?>" style="font-size:10px;height:24px;padding:2px 4px;max-width:160px;">
                            </div>
                            <div class="bm-inputs-row" style="margin-top:4px;">
                                <!-- FEATURE 10: Condizionali -->
                                <input type="text" name="menu_items[<?php echo $idx; ?>][cond_roles]" value="<?php echo esc_attr($item['cond_roles'] ?? ''); ?>" placeholder="Roles (e.g.: editor,author)" title="Show only to these WP roles" style="font-size:10px;height:24px;padding:2px 4px;max-width:140px;">
                                <select name="menu_items[<?php echo $idx; ?>][cond_devices]" style="font-size:10px;height:24px;padding:1px 2px;max-width:100px;" title="Device">
                                ...
                                <input type="text" name="menu_items[<?php echo $idx; ?>][cond_time_from]" value="<?php echo esc_attr($item['cond_time_from'] ?? ''); ?>" placeholder="⏰ From HH:MM" title="Show from" style="font-size:10px;height:24px;padding:2px 4px;max-width:70px;">
                                <input type="text" name="menu_items[<?php echo $idx; ?>][cond_time_to]" value="<?php echo esc_attr($item['cond_time_to'] ?? ''); ?>" placeholder="⏰ To HH:MM" title="Show until" style="font-size:10px;height:24px;padding:2px 4px;max-width:70px;">
                                <input type="text" name="menu_items[<?php echo $idx; ?>][cond_utm]" value="<?php echo esc_attr($item['cond_utm'] ?? ''); ?>" placeholder="UTM source" title="Show only with this utm_source" style="font-size:10px;height:24px;padding:2px 4px;max-width:100px;">
                            </div>
                        </details>
                        <div class="bm-submenu-zone" id="bm-sm-zone-<?php echo $idx; ?>" style="margin-top:6px; border-top:1px dashed #e5e7eb; padding-top:6px;">
                            <button type="button" class="button button-small"
                                onclick="menux_toggleSubmenuZone(<?php echo $idx; ?>)"
                                id="bm-sm-toggle-<?php echo $idx; ?>"
                                style="font-size:11px; background:#f0f6fc; border-color:#c3d4e8; color:#2271b1;">
                                📂 <?php echo $has_subs ? count($item['children']).' sub-items' : 'Add submenu'; ?> ▾
                            </button>
                            <div id="bm-sm-body-<?php echo $idx; ?>" style="display:<?php echo $has_subs ? 'block' : 'none'; ?>; margin-top:8px;">
                                <ul class="bm-sm-list" id="bm-sm-list-<?php echo $idx; ?>" style="list-style:none;padding:0;margin:0 0 6px;border-left:3px solid #c3d4e8;padding-left:10px;">
                                ...
                                <li class="bm-sm-item" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:4px;padding:5px 8px;margin-bottom:4px;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                                    ...
                                    <select name="menu_items[<?php echo $idx; ?>][children][<?php echo $ci; ?>][type]"
                                            class="bm-child-type-sel"
                                            data-parent="<?php echo $idx; ?>" data-child="<?php echo $ci; ?>"
                                    ...
                                    <div class="bm-child-page-wrap" style="display:<?php echo $child_is_page ? 'flex' : 'none'; ?>;align-items:center;gap:4px;">
                                        <select name="menu_items[<?php echo $idx; ?>][children][<?php echo $ci; ?>][id]"
                                                style="font-size:11px;height:24px;padding:1px 4px;max-width:160px;">
                                            <option value="">-- Page --</option>
                                            <?php foreach (get_pages(['sort_column'=>'post_title']) as $pg): ?>
                                            <option value="<?php echo $pg->ID; ?>" <?php selected(intval($child['id']??0), $pg->ID); ?>><?php echo esc_html($pg->post_title); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    ...
                                    <div class="bm-child-url-wrap" style="display:<?php echo !$child_is_page ? 'flex' : 'none'; ?>;align-items:center;gap:4px;">
                                        <input type="text" name="menu_items[<?php echo $idx; ?>][children][<?php echo $ci; ?>][url]" value="<?php echo esc_attr($child_url); ?>" placeholder="https://..." style="font-size:11px;width:130px;height:24px;padding:2px 5px;">
                                    </div>
                                    <?php if (!empty($supported_langs)): foreach ($supported_langs as $cl): $ck = menux_code_to_key($cl['code']); ?>
                                    <input type="text" name="menu_items[<?php echo $idx; ?>][children][<?php echo $ci; ?>][<?php echo esc_attr($ck); ?>]" value="<?php echo esc_attr($child[$ck]??''); ?>" placeholder="<?php echo esc_attr($cl['code']); ?>" title="<?php echo esc_attr($cl['label']); ?>" style="font-size:11px;width:80px;height:24px;padding:2px 5px;">
                                    <?php endforeach; else: ?>
                                    <input type="text" name="menu_items[<?php echo $idx; ?>][children][<?php echo $ci; ?>][lang_it_IT]" value="<?php echo esc_attr($child['lang_it_IT']??$child_label); ?>" placeholder="Label" style="font-size:11px;width:110px;height:24px;padding:2px 5px;">
                                    <?php endif; ?>
                                    <!-- Icona con picker -->
                                    <div style="display:inline-flex;align-items:center;gap:2px;">
                                        ...
                                        <input type="text" name="menu_items[<?php echo $idx; ?>][children][<?php echo $ci; ?>][icon]" value="<?php echo esc_attr($child['icon']??''); ?>" placeholder="icon" class="bm-icon-input" style="font-size:11px;width:90px;height:24px;padding:2px 5px;">
                                    ...
                                    <select name="menu_items[<?php echo $idx; ?>][children][<?php echo $ci; ?>][target]" style="font-size:11px;height:24px;padding:1px 3px;">
                                    ...
                                    <span style="font-size:10px;background:#dbeafe;color:#1d4ed8;padding:1px 5px;border-radius:6px;"><?php echo count($grand_children); ?> children</span>
                                    <?php foreach ($grand_children as $gi => $gc):
                                        $gc_is_page = ($gc['type']==='page');
                                        foreach ($gc as $gk => $gv) {
                                            if ($gk==='children') continue;
                                            echo '<input type="hidden" name="menu_items['.$idx.'][children]['.$ci.'][children]['.$gi.']['.esc_attr($gk).']" value="'.esc_attr($gv).'">';
                                        }
                                    endforeach; ?>
                                    <?php endif; ?>
                                    <button type="button" onclick="this.closest('li').remove(); menux_updateSubCount(<?php echo $idx; ?>);" style="background:#fee2e2;border:1px solid #fca5a5;color:#dc2626;border-radius:3px;padding:1px 7px;font-size:12px;cursor:pointer;margin-left:auto;">&times;</button>
                                </li>
                                <?php endforeach; endif; ?>
                                </ul>
                                <!-- Aggiungi voce figlia -->
                                <div style="display:flex;gap:5px;align-items:center;flex-wrap:wrap;">
                                    <button type="button" onclick="menux_addSubBlank(<?php echo $idx; ?>)" class="button button-small" style="font-size:11px;background:#f0f6fc;border-color:#c3d4e8;color:#2271b1;">+ Add sub-item</button>
